...
class SequenceLightningModule(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        if len(z) == 0:
            ...
            assert len(z) == 1 and isinstance(z[0], dict), f"Dataloader must return dictionary of extra arguments (e.g. sequence length), got {z})"
            z = z[0]
...
def train(config):
    ...
    if config.train.get("pretrained_model_path", None) is not None:
        ...
        if config.train.get("freeze_pretrained_layers", False):
            for name, param in model.named_parameters():
                for freeze_layer in config.train.freeze_pretrained_layers:
