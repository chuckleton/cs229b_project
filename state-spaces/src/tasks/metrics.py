...
mrstft = auraloss.freq.MultiResolutionSTFTLoss(
    sample_rate = 1_000.0 / 0.5,
    fft_sizes=[128, 256, 512],
    hop_sizes=[15, 30, 50],
    win_lengths=[75, 150, 240],
)
...
def pulse_loss(outs, y, len_batch=None):
    mse_loss = mse(outs, y, len_batch)
    # print(outs.shape, y.shape)
    mrstft_loss = mrstft(outs.transpose(-1, -2), y.transpose(-1, -2))
    mse_scaling = 1.0
    mrstft_scaling = 1.0 / 4.0
    # print(f"mse_loss: {mse_loss*mse_scaling }, mrstft_loss: {mrstft_loss*mrstft_scaling}")
    return mse_loss*mse_scaling + mrstft_loss*mrstft_scaling

def mrstft_loss(outs, y, len_batch=None):
    return mrstft(outs.transpose(-1, -2), y.transpose(-1, -2))

def full_test_loss(outs, y, len_batch=None):
    # print(f"{len_batch=}, {outs=}, {y=}, {torch.mean((outs - y)**2)}, {mse(outs, y, len_batch)}")
    mse_loss = mse(outs, y, len_batch)
    # return mse_loss
    # return mse_loss
    # print(outs.shape, y.shape)
    mrstft_loss_value = mrstft_loss(outs, y, len_batch)
    mse_scaling = 100.0
    mrstft_scaling = 1.0
    # print(f"mse_loss: {mse_loss*mse_scaling }, mrstft_loss: {mrstft_loss*mrstft_scaling}")
    return mse_loss*mse_scaling + mrstft_loss_value*mrstft_scaling
