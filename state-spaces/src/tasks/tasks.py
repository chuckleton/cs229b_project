...
class PulseTask(BaseTask):
    def __init__(
        self,
        sample_time: float,
        pre_sample_time: float,
        sample_rate: float,
        **kwargs,
    ):
        super().__init__(**kwargs)
    ...
    def metrics(self, x, y, **kwargs):
        return super().metrics(x[:, -1], y[:, -1], **kwargs)
...
class FullTestTask(BaseTask):
    def __init__(
        ...
        **kwargs,
        ...
        import copy
        loss_fn = copy.deepcopy(self.loss)
        pre_sample_samples = int(pre_sample_time * sample_rate)
        def new_loss(x,y):
            # print(f"{x.shape=},{y.shape=},{pre_sample_samples=}")
            return loss_fn(x[:, pre_sample_samples:], y[:, pre_sample_samples:])
        self.loss = new_loss
        self.loss = U.discard_kwargs(self.loss)  # remove extra kwargs
        if hasattr(self, 'loss_val'):
            loss_val_fn = copy.deepcopy(self.loss_val)
            self.loss_val = lambda x, y: loss_val_fn(x[:, pre_sample_samples:], y[:, pre_sample_samples:])
            self.loss_val = U.discard_kwargs(self.loss_val)
    ...
    def metrics(self, x, y, **kwargs):
        pre_sample_samples = int(self.pre_sample_time * self.sample_rate)
        return super().metrics(x[:, pre_sample_samples:], y[:, pre_sample_samples:], **kwargs)

class FullTestLastItemTask(BaseTask):
    def __init__(
        self,
        sample_time: float,
        pre_sample_time: float,
        ...
        **kwargs,
        ...
        pre_sample_samples = int(pre_sample_time * sample_rate)
        def new_loss(x,y):
            print(f"{x[:, -1]=},{y[:, -1]=},{loss_fn(x[:, -1], y[:, -1])=}")
            return loss_fn(x[:, -1], y[:, -1])
        self.loss = new_loss
        self.loss = U.discard_kwargs(self.loss)  # remove extra kwargs
        if hasattr(self, 'loss_val'):
            loss_val_fn = copy.deepcopy(self.loss_val)
...
registry = {
    'base': BaseTask,
    'pulse': PulseTask,
    'full_test': FullTestTask,
    'full_test_last_item': FullTestLastItemTask,
    'lm': LMTask,
